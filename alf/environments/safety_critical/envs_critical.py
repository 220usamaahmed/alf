...
class GlucoseHistory:
    ...
    def get_obs(self):
        return np.array(
            [self.obs_queue[1], self.obs_queue[1] - self.obs_queue[0], self.time]
        )
...
class Glucose(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.observation_space = Box(
            ...
            dtype=np.float64,  # Data type of the observation space
        )
        ...
        self.init_state = np.array([self.Gb, 0.0, self.Ib], dtype=np.float32)
    ...
    def step(self, action: np.ndarray) -> Union[np.ndarray, float, bool, bool, Dict]:
        ...
        if G < 10:
            cost = (10 - G) / 10  # Normalized to [0,1]
        elif G > 1000:
            cost = (G - 1000) / 1000  # Normalized to [0,1]
        else:
            cost = 0.0
        ...
        return (
            ...
            term,
            trun,
            {"state": self.state, "meas": np.array([G]), "cost": cost},
        )
    ...
    def get_plot_obs(self, eps_obs: np.ndarray):
        assert (
            eps_obs.ndim > 1
        ), "_get_plot_obs() deals with obs with shape (N, num_obs)"
        return eps_obs[:, 0]
...
class BiGlucose(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Union[np.ndarray, float, bool, bool, Dict]:
        ...
        # print(trun, self.time_step, self.max_steps)
    ...
    def _solve_steady_state(self):
        ...
        b = -(
            (self.EGP_0 * self.k_b3) / (self.k_a3 * self.k_e * self.V_I)
        ...
        Q_2 = (7.7 * self.V_G * self.k_b1 * self.k_a2 * k) / (
            self.k_a1 * self.k_a2 * self.k_12 * self.k_e * self.V_I
            + self.k_b2 * self.k_a1 * k
        )
        ...
        x_steady_state = np.array(
            [Q_1, Q_2, x_1, x_2, x_3, S_1, S_2, I, 0.0, 0.0, self.N_b, 0.0],
            dtype=np.float32,
        )
        u_steady_state = np.array([k, 0.0], dtype=np.float32)
...
class CSTR(gym.Env):
    ...
    def __init__(
        ...
        self.cost_space = gym.spaces.Box(
            ...
            high=np.inf * np.ones(3, dtype=np.float32),
            dtype=np.float32,
        )
    ...
    def step(self, action: np.ndarray) -> Union[np.ndarray, float, bool, bool, Dict]:
        ...
        term = (
            (C_a < 0.1 or C_a > 2)
            or (C_a < 0.1 or C_a > 2)
            or (T_R < 50 or T_R > 200)
            or (T_K < 50 or T_K > 150)
        )
        trun = True if self.time_step >= self.max_steps else False
        ...
        if T_R < 50 or T_R > 200:
            c2 = 1.0
        else:
            c2 = 0.0
        if T_K < 50 or T_K > 150:
            c3 = 1.0
        else:
            c3 = 0.0

        cost = np.sum([c1, c2, c3])

        # reward = np.concatenate([[reward], cost])
        ...
        return (
            ...
            term,
            trun,
            {"state": self.state, "meas": self.state, "cost": cost},
        )
...
class CartPole(CartPoleEnv):
    ...
    def step(self, action):
        ...
        temp = (
            action.item() * self.force_mag
            + self.polemass_length * theta_dot**2 * sintheta
        ) / self.masstotal
        thetaacc = (self.gravity * sintheta - costheta * temp) / (
            self.length * (4.0 / 3.0 - self.masspole * costheta**2 / self.masstotal)
        )
        ...
        if x < -4.8 or x > 4.8:
            ...
            c1 = 0.0
        if theta < -24 * math.pi / 360 or theta > 24 * math.pi / 360:
            c2 = 1.0
        else:
            c2 = 0.0
        ...
        return (
            ...
            terminated,
            truncated,
            {"state": self.state, "meas": self.state},
        )
